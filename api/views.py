...
class ObservationList(APIView):
    def get(self, request, format=None):
        ...
        if from_date and to_date:
            observations = observations.filter(send_date__range=(from_date, to_date))
        if phenomenon:
            observations = observations.filter(values__phenomenon__name__iexact=phenomenon)
        if phenomenon_id:
            observations = observations.filter(values__phenomenon__id=phenomenon_id)
        if category:
            observations = observations.filter(values__phenomenon__category__name__iexact=category)
        if category_id:
            observations = observations.filter(values__phenomenon__category__id=category_id)
